...
type Step = "capture" | "details" | "success" | "history";
...
export default function FieldOfficerPage() {
  ...
  return <RoleGuard role="FIELD_OFFICER"><main className="mobile-workspace field-workspace"><PwaRegister/>
    <header className="mobile-header field-header"><div className="mobile-brand"><ShieldCheck/><span><b>NER-LOGIX FIELD</b><small>Authenticated Field Officer</small></span></div><button onClick={()=>router.push("/")} aria-label="Sign out"><LogOut/></button></header>
    <div className="connectivity"><span className={online?"online":"offline"}>{online?<Radio/>:<CloudOff/>}{online?"Online · sync active":"Offline · reports queue locally"}</span><span>{pending} pending</span></div>
    <nav className="field-mobile-tabs"><button className={step==="capture"?"active":""} onClick={()=>setStep("capture")}><MapIcon/>Sector Map</button><button className={step==="details"?"active":""} onClick={()=>setStep("details")}><PlusCircle/>Report</button><button className={step==="history"?"active":""} onClick={()=>setStep("history")}><History/>History</button></nav>
    ...
    {step==="capture"&&<section className="field-capture">
      <div className="assignment-banner"><FileCheck2/><span><small>GROUND VERIFICATION</small><b>No road condition is pre-assumed</b></span></div>
      <h1>Sector accessibility map</h1><p>Review the affected corridor. GPS is optional and can be attached from the report form.</p>
      ...
      <button className="location-button" onClick={captureLocation}><LocateFixed/>{location?"REFRESH CURRENT LOCATION":"USE CURRENT LOCATION"}</button>{locationError&&<p className="form-error">{locationError}</p>}
      <button className="field-primary" onClick={()=>setStep("details")}>OPEN INCIDENT REPORT</button>
    </section>}

    {step==="details"&&<section className="mobile-form-page">
      <button className="back-button" onClick={()=>setStep("capture")}><ArrowLeft/>Back to sector map</button><h1>Report road condition</h1><p>Only the description is required. GPS and photo evidence are optional and help the Control Room verify the report.</p>
      <div className="form-grid">
        <label>Road segment<select value={form.road_id} onChange={e=>setForm({...form,road_id:e.target.value})}>{["R-01","R-02","R-03","R-04","R-05","R-06","R-07","R-08"].map(id=><option key={id}>{id}</option>)}<option value="UNMAPPED">Road unknown — use landmark</option></select></label>
        <label>Inspection type<select value={form.incident_type} onChange={e=>setForm({...form,incident_type:e.target.value})}><option value="FLOOD">Flood</option><option value="LANDSLIDE">Landslide</option><option value="BLOCKAGE">Road blockage</option><option value="BRIDGE_DAMAGE">Bridge damage</option><option value="OTHER">Other</option></select></label>
        ...
        <label>Road status<select value={form.road_status} onChange={e=>setForm({...form,road_status:e.target.value})}><option value="SAFE">Safe</option><option value="CAUTION">Passable with caution</option><option value="RESTRICTED">Restricted</option><option value="BLOCKED">Completely blocked</option></select></label>
        ...
        <label>Affected direction<select value={form.affected_direction} onChange={e=>setForm({...form,affected_direction:e.target.value})}><option value="BOTH">Both directions</option><option value="EASTBOUND">Eastbound</option><option value="WESTBOUND">Westbound</option></select></label>
        <label>Vehicle access<select value={form.vehicle_access} onChange={e=>setForm({...form,vehicle_access:e.target.value})}><option value="ALL">All vehicles</option><option value="LIGHT_ONLY">Light vehicles only</option><option value="EMERGENCY_ONLY">Emergency vehicles only</option><option value="NONE">No vehicles</option></select></label>
        <label>Estimated clearance<select value={form.clearance_estimate} onChange={e=>setForm({...form,clearance_estimate:e.target.value})}><option>Under 1 hour</option><option>1–3 hours</option><option>More than 3 hours</option><option>Unknown</option></select></label>
      </div>
      <label>Description<textarea required rows={5} value={form.description} onChange={e=>setForm({...form,description:e.target.value})} placeholder="Describe only what you observed"/></label>
      <section className="optional-evidence"><div><small>OPTIONAL EVIDENCE</small><b>Add GPS or a road photo</b></div><button className="report-location-button" onClick={captureLocation}><LocateFixed/>{location?"REFRESH GPS":"ADD CURRENT LOCATION"}</button>{locationError&&<p className="form-error">{locationError}</p>}<label className="photo-drop compact-photo-drop">{preview?<img src={preview} alt="Incident evidence preview"/>:<><Camera/><b>Upload or capture photo</b><small>Optional · JPG, PNG or WebP · maximum 5 MB</small></>}<input type="file" accept="image/jpeg,image/png,image/webp" capture="environment" onChange={event=>choosePhoto(event.target.files?.[0])}/></label>{(location||photo)&&<div className="evidence-summary">{preview&&<img src={preview} alt="Attached evidence"/>}<span>{location?<><MapPin/>GPS accuracy ±{location.accuracy} m</>:<><MapPin/>Using selected road coordinates</>}<br/>{photo?<><Upload/>Photo ready to {online?"upload":"submit when online"}</>:<><Upload/>No photo attached</>}</span></div>}</section>{result?.error&&<p className="form-error">{result.error}</p>}
      <button className="field-primary" onClick={submit} disabled={busy||!form.description.trim()||!customFieldsReady||!inspectionTypeReady}>{busy?"SUBMITTING…":"SUBMIT REPORT FOR REVIEW"}</button>
    </section>}

    {step==="success"&&<section className="success-screen field-success"><CheckCircle2/><h1>{result?.queued?"Saved offline":"Report synchronized"}</h1><p>{result?.queued?"The report and optional photo will sync automatically when connectivity returns.":"Control Room review is pending. Confirmed closures will trigger immediate rerouting."}</p><div className="report-receipt"><span><small>INCIDENT ID</small><b>{result?.id||"PENDING"}</b></span><span><small>STATUS</small><b>{result?.queued?"PENDING SYNC":"PENDING REVIEW"}</b></span><span><small>ROAD</small><b>{form.road_id}</b></span><span><small>TIME</small><b>{new Date().toLocaleTimeString()}</b></span></div><button className="field-primary" onClick={()=>{setStep("capture");setPhoto(null);setPreview("");setLocation(null);setForm({...form,description:""})}}>NEW VERIFICATION</button></section>}
  </main></RoleGuard>;
}
